...
def read_modules(devices, mac_channels, sensors, header):
    ...
    #print(t[0,:])
    #print(t_display[0,:])
    #print('shape t: {} | shape display: {}'.format(t.shape, t_display.shape))
#         else:
#             #t = np.concatenate((t, t_read), axis=1)
#             
#             n = 0
#             for j,chn in enumerate(mac_channels):
#                 if chn[0] == device.macAddress:
#                     r = resolution[n]
#                     signal_type = sensors[j]
#                     display_aux = get_transform(t_nseq[:,n], signal_type, r) # receives and returns a column with 100 samples
#                     n += 1
#                     display_aux = np.concatenate((display_aux, display_aux), axis=1)
#             
#             t = np.concatenate((t, display_aux), axis=1)
            #display_aux = np.concatenate((display_aux, t_read[:,1:]), axis=1)
    
    #print('t: {}'.format(t))
    #print('disp: {}'.format(display_aux))
